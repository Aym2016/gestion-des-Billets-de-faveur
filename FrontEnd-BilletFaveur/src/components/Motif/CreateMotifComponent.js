...
export default class CreateMotifComponent extends Component {
    constructor(props) {
        super(props)
        ...
        this.saveOrUpdateMotif = this.saveOrUpdateMotif.bind(this);
    }
    ...
    componentDidMount(){
        ...
        if(this.state.id === '_add'){
            ...
            MotifAction.getMotifById(this.state.id).then( (res) =>{
                let motif = res.data;
                this.setState({code: motif.code, libelle: motif.libelle
                });
            });
        }        
    }
    saveOrUpdateMotif = (e) => {
        e.preventDefault();
        let motif = {code: this.state.code, libelle: this.state.libelle};
        console.log('motif => ' + JSON.stringify(motif));

        // step 5
        if(this.state.id === '_add'){
            MotifAction.createMotif(motif).then(res =>{
                this.props.history.push('/motifs');
            });
        }else{
            MotifAction.updateMotif(motif, this.state.id).then( res => {
                this.props.history.push('/motifs');
            });
        }
    }
    AddMotif = (event) => {
        ...
        Axios.post(URL + '/motif/save', this.state)
            .then(response => {
    ...
    getTitle(){
        if(this.state.id === '_add'){
            return <h3 className="text-center">Add motif</h3>
        }else{
            return <h3 className="text-center">Update motif</h3>
        }
    }
    render() {        
        return (
                ...
                <div className="container pad">
                    <div className="row">
                        <div className="card col-md-6 offset-md-3 offset-md-3">
                            ...
                            <form onSubmit={this.saveOrUpdateMotif}>
